...
@section('content')
                ...
                <form>
                    <div class="portlet box">
                        <div class="portlet-header">
                            ...
                            <div class="actions">
                                <a href="{{url('dvvantai/dvxtx/print/'.$modelkk->id)}}" target="_blank" class="btn btn-info"><i class="fa fa-print"></i> In kê khai </a>
                            </div>
                        </div>
                                                        ...
                                                        @if(session('admin')->level == 'H')
                                                            @if($modelkk->trangthai == 'Chờ chuyển')
                                                                @if(isset($giadv->idgia))
                                                                    <a href="{{url('dvvantai/dvxtx/chitiet/edit/'.$modelkk->id.'/'.$giadv->idgia.'')}}" class="btn btn-info btn-xs mbs">&nbsp;Chỉnh sửa</a>
                                                                ...
                                                                @else
                                                                    <a href="{{url('dvvantai/dvxtx/chitiet/create/'.$modelkk->id.'/'.$giadv->madichvu)}}" class="btn btn-success btn-xs mbs"><i class="fa fa-plus"></i>&nbsp;Nhập giá</a>
                                                                @endif
                                                            @endif
                                                        @endif
                                                    </td>
    ...
    <div id="delete-modal-confirm" tabindex="-1" role="dialog" aria-hidden="true" class="modal fade">
        {!! Form::open(['url'=>'dvvantai/dvxtx/chitiet/del/'.$modelkk->id,'id' => 'frm_delete','files'=>true, 'class'=>'form-horizontal form-validate']) !!}
